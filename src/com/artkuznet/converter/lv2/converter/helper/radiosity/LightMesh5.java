...
public class LightMesh5 implements LightProcessor {
    @Override
    public void process(Mesh mesh, List<Vector3D> vertices, List<Material> materials, List<LdbMaterial> ldbMaterials, int materialId) {
        ...
        Polygon p = mesh.getPolygons().stream()
                .filter(polygon -> polygon.getNorm1().getY() > 0)
                .max(Comparator.comparingDouble(Polygon::getArea)).orElseThrow(RuntimeException::new);

        int id = mesh.getPolygons().indexOf(p);

        RadiosityLight light = new RadiosityLight(255, 255, 255, 500, 180, 180);
